...
python gr-lora_sdr\examples\lora_file_RX.py `
  ...
  --sf 12 --bw 125000 --samp-rate 500000 --cr 1 `
  --center-freq 487.7e6 --sync-word 0x34 `
  --has-crc --crc-mode 1
...
`examples/lora_file_RX.py` 现在支持把 `frame_sync` 内部已经同步、校正后的前导码 IQ 画成频谱图。这个功能用于检查前导码对齐效果、SF/采样率是否匹配，也方便后续继续做信号分析。

### 7.1 设计边界

- C++ 层 `frame_sync_impl.cc` 只负责导出可信的对齐前导码 IQ 数据和同步元信息。
- Python 层 `lora_file_RX.py` 负责接收 IQ、画图、保存 PNG。后续如果要导出 `.npy/.npz` 或做更复杂分析，也应该继续放在 Python 层。
- C++ 不做绘图，避免把 matplotlib/numpy 分析逻辑塞进实时信号处理块。
...
这里使用 `pmt::init_c32vector(...)`，而不是 `pmt::make_blob(...)`。原因是 Windows GNU Radio Python 绑定中 `pmt.blob_data()` 可能返回 `PyCapsule`，而 `c32vector` 可以在 Python 中用 `pmt.c32vector_elements()` 稳定还原。

### 7.3 Python 脚本参数

`lora_file_RX.py` 新增参数：
