...
From `d:\Desktop\proj` using the `radioconda` GNU Radio environment:

```powershell
& 'D:\mysoft2\radioconda\python.exe' '.\dong\multipath\capture_sweep.py' --output '.\dong\multipath\runs\cal_loopback.npz' --tx-enable --center 487.7e6 --span 2e6 --step 125e3 --samp-rate 500e3 --rf-bandwidth 500e3 --duration 0.08 --rx-gain 0.5 --tx-gain 0.15 --tx-amplitude 0.05
```

Then analyze it:

```powershell
& 'D:\mysoft2\radioconda\python.exe' '.\dong\multipath\analyze_sweep.py' --input '.\dong\multipath\runs\cal_loopback.npz' --output-dir '.\dong\multipath\runs\cal_loopback_analysis'
```

For an antenna measurement, keep the same sweep grid:

```powershell
& 'D:\mysoft2\radioconda\python.exe' '.\dong\multipath\capture_sweep.py' --output '.\dong\multipath\runs\ota_measurement.npz' --tx-enable --center 487.7e6 --span 2e6 --step 125e3 --samp-rate 500e3 --rf-bandwidth 500e3 --duration 0.08 --rx-gain 0.5 --tx-gain 0.15 --tx-amplitude 0.05
```

Analyze with calibration:

```powershell
& 'D:\mysoft2\radioconda\python.exe' '.\dong\multipath\analyze_sweep.py' --input '.\dong\multipath\runs\ota_measurement.npz' --calibration '.\dong\multipath\runs\cal_loopback.npz' --output-dir '.\dong\multipath\runs\ota_measurement_analysis'
```
...
```powershell
& 'D:\mysoft2\radioconda\python.exe' '.\dong\multipath\capture_sweep.py' --output '.\dong\multipath\runs\cal_20mhz.npz' --tx-enable --center 487.7e6 --span 20e6 --step 250e3 --samp-rate 1e6 --rf-bandwidth 1e6 --probe-bandwidth 400e3 --duration 0.08
```
