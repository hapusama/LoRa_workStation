    ...
    case MODEM_LORA:
        {
            ...
            else
            {
                SX1276Write( REG_LR_IRQFLAGSMASK, //RFLR_IRQFLAGS_RXTIMEOUT |
                                                  //RFLR_IRQFLAGS_RXDONE |
                                                  //RFLR_IRQFLAGS_PAYLOADCRCERROR |
                                                  //RFLR_IRQFLAGS_VALIDHEADER | // modified(removed) this sentence to enable vaildheader irq. 2022.06.02
                                                  RFLR_IRQFLAGS_TXDONE |
                                                  RFLR_IRQFLAGS_CADDONE |
                                                  RFLR_IRQFLAGS_FHSSCHANGEDCHANNEL |
                                                  RFLR_IRQFLAGS_CADDETECTED );

                // DIO0=RxDone, DIO3=ValidHeader(add by xzq 2022.06.02)
                //SX1276Write( REG_DIOMAPPING1, ( SX1276Read( REG_DIOMAPPING1 ) & RFLR_DIOMAPPING1_DIO0_MASK ) | RFLR_DIOMAPPING1_DIO0_00 ); // replaced by below. 2022.06.02
...
uint16_t SX1276GetValidHeaderCnt( void ) // addd by xzq. 2022.06.03
{
		return (SX1276Read(REG_LR_RXHEADERCNTVALUEMSB) << 8) + SX1276Read(REG_LR_RXHEADERCNTVALUELSB);
}

uint16_t SX1276GetValidPacketCnt( void ) // addd by xzq. 2022.06.03
{
		return (SX1276Read(REG_LR_RXPACKETCNTVALUEMSB) << 8) + SX1276Read(REG_LR_RXPACKETCNTVALUELSB);
}
...
void SX1276OnDio0Irq( void* context )
{
    volatile uint8_t irqFlags = 0;
		volatile bool isCRCError = false;
            ...
            case MODEM_LORA:
                {
                    ...
                    isCRCError = false; // add by xzq 2022.06.02
										irqFlags = SX1276Read( REG_LR_IRQFLAGS );
                    if( ( irqFlags & RFLR_IRQFLAGS_PAYLOADCRCERROR_MASK ) == RFLR_IRQFLAGS_PAYLOADCRCERROR ) // payload crc error occurs
                    {
												isCRCError = true; // add by xzq 2022.06.02
                        ...
                        /*if( ( RadioEvents != NULL ) && ( RadioEvents->RxError != NULL ) ) // removed by xzq 2022.06.02
                        {
                            RadioEvents->RxError( );
                        }
                        break;*/
										...
										if( ( RadioEvents != NULL ) && ( RadioEvents->PayloadCRCError != NULL ) ) // add by xzq 2022.06.02
										{
												RadioEvents->PayloadCRCError(isCRCError);
										}
		...
		switch( SX1276.Settings.State )
		{
				case RF_RX_RUNNING:
						...
						case MODEM_FSK:
								break;
						case MODEM_LORA:
								if( ( SX1276Read( REG_LR_IRQFLAGS ) & RFLR_IRQFLAGS_VALIDHEADER_MASK ) == RFLR_IRQFLAGS_VALIDHEADER ) // valid header
								{
										// Clear Irq
										SX1276Write( REG_LR_IRQFLAGS, RFLR_IRQFLAGS_VALIDHEADER );
										if( ( RadioEvents != NULL ) && ( RadioEvents->ValidHeader != NULL ) )
										{
												RadioEvents->ValidHeader( );
										}
								}
								break;
						default:
						...
						break;
				case RF_CAD:
						switch( SX1276.Settings.Modem ) // origin code start
						{
						case MODEM_FSK:
								break;
						case MODEM_LORA:
